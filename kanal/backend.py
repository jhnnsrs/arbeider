...
class KanalBackend(PodBackendRegister):
    persistent = True
    provider = "kanal"
    register = KanalPod
    _channel = None
    ...
    def catalog_class(self, cls,**kwargs):
        ...
        podDefaults.update(additionalKwargs)
        podUniques.update(additionalUniques)

        if not bool(podUniques):
        ...
        if issubclass(self.konfig, Konfig): # A Check
            ...
            try:
                pod = register.objects.get(**podUniques)
                for key, value in podDefaults.items():
                    setattr(pod, key, value)
                pod.save()
                logger.debug(f"Updated Pod on Node {node.name}")
            except KanalPod.DoesNotExist:
                combined = {**podUniques}
                combined.update(podDefaults)
    ...
    def register_class(self, cls):
        get_kanal_registry().registerChannelConsumer(self.get_channel(), cls)
...
class register_with_kanal_backend(KanalBackend):
    pass